  ...
  return (
    ...
    >
      ...
      <div className="lib-filter-tabs mb-6">
        {['Барлығы', 'ҚМЖ', 'Ойын', 'БЖБ/ТЖБ'].map((t, i) => (
          <button 
            key={i} 
        ...
        <AnimatePresence mode="popLayout">
          {filtered.length === 0 ? (
            ...
            filtered.map((item, i) => (
              ...
              >
                <div className="lib-card-icon">{item.type === 'ҚМЖ' ? '📄' : '🎮'}</div>
            ...
            {viewItem.type === 'ҚМЖ' ? (
              <div className="p-4">
                <KMZHView 
                  ...
                  loading={false}
                  setLoading={() => {}}
                  result={viewItem.data}
                  setResult={() => {}}
                  params={{ subject: viewItem.subject, grade: viewItem.grade, topic: viewItem.title, language: 'Kazakh' }}
                  setParams={() => {}}
                  loaderStep={0}
                  steps={[]}
