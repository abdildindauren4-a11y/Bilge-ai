...
const WebGame: React.FC<{ data: any; viewSize: 'mobile' | 'tablet' | 'desktop' }> = ({ data, viewSize }) => {
  ...
  useEffect(() => {
    if (iframeRef.current && data) {
      // Use a more robust way to inject content to avoid template literal issues with backticks
      ...
      // To handle backticks in jsContent properly within the template literal, 
      // we can't easily escape them all if they are complex.
      // A better way is to use a blob or just set srcdoc carefully.
      // For now, let's try to just use srcdoc but be aware of the limitation.
      // If the user has backticks, we might need to use a different injection method.
...
export const CodingView: React.FC<CodingViewProps> = ({ isApiOk, isClaudeApiOk, onOpenApiModal, onOpenClaudeApiModal, addNotification }) => {
  ...
  const handleSend = async (customMsg?: string) => {
    ...
    if (!isApiOk && !isClaudeApiOk) return onOpenApiModal();
    ...
    try {
      ...
      if (isClaudeApiOk) {
        try {
          result = await generateGameWithClaude(userMsg, chatHistory, gameData);
        } catch (claudeErr: any) {
          ...
          if (isApiOk) {
            ...
            result = await generateGameIterative(userMsg, chatHistory, gameData);
  ...
  const saveGame = async (asTemplate: boolean = false) => {
    ...
    try {
      ...
      console.error(err);
      addNotification('Қате ❌', 'Сақтау кезінде қате шықты. Қайта көріңіз.', 'error');
    }
  };
  ...
  const shareGame = async () => {
    ...
    if (!id) {
      ...
      try {
        ...
        console.error(err);
        addNotification('Қате', 'Сілтеме жасау мүмкін болмады.', 'error');
        return;
      }
    }
                ...
                >
                  {!gameData ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      ...
                      {!['kahoot', 'flashcards', 'matching', 'web'].includes(gameData.type) && (
