...
import { CheckCircle2, Send, User, ChevronRight, ChevronLeft } from 'lucide-react';
...
export const StudentAssessmentView: React.FC<StudentAssessmentViewProps> = ({ addNotification }) => {
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 md:p-8">
        <div className="max-w-2xl mx-auto">
          ...
          <button className="btn btn-primary w-full mt-8" onClick={() => window.location.href = getBaseUrl()}>Басты бетке өту</button>
        </div>
      </div>
    );
  }
        ...
        <div className="card card-pad max-w-md w-full">
          ...
          {assessment.metadata.students && assessment.metadata.students.length > 0 && (
            <div className="fg mb-8">
              ...
              <input 
                type="text" 
                className="inp text-center font-mono font-bold tracking-widest uppercase" 
                placeholder="XXXXXX" 
                maxLength={6}
                value={accessCode}
