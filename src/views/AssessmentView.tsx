...
export const AssessmentView: React.FC<AssessmentViewProps> = ({ isApiOk, onOpenApiModal, addNotification, result: propResult, setResult: propSetResult, onNavigate, t }) => {
  const [loading, setLoading] = useState(false);
  const [resultState, setResultState] = useState<AssessmentData | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    if (!isApiOk) {
      addNotification('API Кілті қажет 🔑', 'Алдымен профиль бөлімінде Gemini API кілтін орнатыңыз.', 'error');
      return;
    }
    if (!params.topic) {
    ...
    try {
      ...
      const data = await generateAssessment(generationParams);
      
      // Generate student access codes if online
      if (params.mode === 'online' && params.studentsList) {
        const students = params.studentsList.split('\n').filter(s => s.trim()).map(name => ({
          name: name.trim(),
          accessCode: Math.random().toString(36).substring(2, 8).toUpperCase(),
          status: 'pending' as const
        }));
        data.metadata.students = students;
      }
      ...
      data.metadata.totalPoints = total;
      
      setResult(data);
      addNotification(`${params.type} Дайын! 📝`, `${params.topic} бойынша тапсырмалар сәтті жасалды.`, 'success');
      
      if (params.mode === 'online') {
        await saveToLibrary(data, true);
      }
    } catch (err: any) {
      console.error("Assessment Generation Error:", err);
      let errorMsg = 'Тапсырмалар жасау кезінде қате шықты. Қайта көріңіз.';
  ...
  const saveToLibrary = async (dataToSave = result, isOnline = false) => {
    ...
    try {
      ...
      addNotification('Сақтау қатесі ❌', 'Деректер қорына сақтау мүмкін болмады.', 'error');
    }
  };
    ...
    >
        ...
        <>
          <div className="card card-pad mb-8">
            ...
            <div className="space-y-6">
              {!isApiOk && (
                <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-2xl p-4 mb-6 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-sm text-orange-800 dark:text-orange-200">{t.apiKey} қажет</h4>
                      <p className="text-xs text-orange-700 dark:text-orange-300">БЖБ/ТЖБ жасау үшін Gemini API кілтін енгізіңіз.</p>
                    </div>
                  </div>
      ...
      {loading && (
        <div className="ai-loader show">
          ...
          <h3 className="ai-loader-title">{t.assessment_view.loader_title}</h3>
          <p className="ai-loader-sub">{t.assessment_view.loader_sub}</p>
        </div>
      )}
