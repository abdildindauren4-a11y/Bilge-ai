...
export const MatchingGame: React.FC<MatchingGameProps> = ({ onBack, data }) => {
  ...
  useEffect(() => {
    ...
    setLeftItems(pairs.map((p: any, i: number) => ({ id: i, text: p.left })));
    setRightItems([...pairs].map((p: any, i: number) => ({ id: i, text: p.right })).sort(() => Math.random() - 0.5));
  }, [data]);
      ...
      <div className="match-grid">
        <div className="match-col">
          {leftItems.map(item => (
            <div 
              key={item.id} 
              className={`match-item ${selectedLeft === item.id ? 'selected' : ''} ${matched.includes(item.id) ? 'matched' : ''} ${wrong === item.id ? 'wrong' : ''}`}
        ...
        <div className="match-col">
          {rightItems.map(item => (
            <div 
              key={item.id} 
              className={`match-item ${selectedRight === item.id ? 'selected' : ''} ${matched.includes(item.id) ? 'matched' : ''}`}
